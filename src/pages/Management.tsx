...
const Manage = () => {
  ...
  const getUsageColor = (level: number) => {
    ...
    return 'bg-green-400';                     // 100%까지: 청록/민트 (자주 사용)
  };
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-primary text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-teal-100 hover:scale-105 transition-all"
        >
          <span className="text-xl">+</span> 구독 추가
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSubs.map(sub => (
          <div key={sub.id} className="bg-white p-7 rounded-[32px] shadow-sm border border-gray-50 flex flex-col justify-between hover:shadow-md transition-shadow relative">
            <div>
              <div className="flex justify-between items-start mb-4">
                <span className="bg-teal-50 text-primary text-[10px] px-2.5 py-1 rounded-lg font-bold">
                  {sub.category}
                </span>
                <div className="flex gap-3">
                  <button onClick={() => handleOpenEdit(sub)} className="opacity-40 hover:opacity-100 transition-opacity">
                    <img src="/src/assets/edit_icon.svg" alt="edit" className="w-5 h-5" />
                    {/* 아이콘 파일이 없다면 '✎' 로 대체 가능 */}
                  </button>
                  <button onClick={() => deleteSubscription(sub.id)} className="opacity-40 hover:opacity-100 transition-opacity">
                    <img src="/src/assets/delete_icon.svg" alt="delete" className="w-5 h-5" />
                    {/* 아이콘 파일이 없다면 '🗑' 로 대체 가능 */}
                  </button>
                </div>
              </div>

              <h3 className="text-2xl font-bold text-gray-800 mb-1">{sub.name}</h3>

              <div className="flex items-baseline gap-2 mt-4">
                <span className="text-3xl font-black text-primary">
                  {(sub.price / sub.sharedPeople).toLocaleString()}원
                </span>
                <span className="text-xs text-gray-400 font-medium">{sub.sharedPeople}명 공유</span>
              </div>
            ...
            <div className="mt-8 space-y-2">
              <div className="flex justify-between items-end">
                <span className="text-xs text-gray-400 font-bold">이용률</span>
                <span className="text-xs text-gray-800 font-bold">{sub.usageLevel}%</span>
              </div>
